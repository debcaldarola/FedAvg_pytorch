...
function run_fedavg() {
	...
	pushd models/
		python main.py -dataset 'femnist' -model 'cnn' --num-rounds ${num_rounds} --clients-per-round ${clients_per_round} --num-epochs ${num_epochs} -lr ${fedavg_lr} -device 'cuda' --batch-size 10 --eval-every 100
	popd
